...
2. Boot selection — указать ISO-образ установщика (не Live-образ!), имя файла должно начинаться на `installation-...`, а не содержать `live`.
3. Partition scheme — **MBR**, Target system — **BIOS or UEFI** (универсальный вариант для любого железа).
4. Если ISO весит больше 4 ГБ — при нажатии **START** Rufus может предложить выбор режима записи. Выбрать **DD Image mode** (не ISO Image mode) — это обходит ограничение FAT32 на размер файла в 4 ГБ, так как образ пишется побайтово, а не копированием файлов.
5. Дождаться завершения записи (статус READY → процесс → READY).

---

## Часть 1. Установка Astra Linux SE 1.8 (сервер и каждый клиент — одинаково)

1. Загрузиться с флешки (в BIOS/UEFI выставить приоритет загрузки с USB).
2. На экране загрузочного меню — если оно появляется — выбрать пункт **«Install»** (не Live, не Graphical install с лишним функционалом, если есть выбор). Если меню сразу грузит инсталлятор — это нормально для установочного (не Live) образа.
3. Пройти шаги инсталлятора:
   - **1. Добро пожаловать** — выбрать язык
   - **2. Лицензия** — принять
   - **3. Региональные настройки** — часовой пояс, раскладка
   - **4. Компоненты установки** — см. ниже отдельный блок, это ключевой шаг
   - **5. Пользователи** — задать root-пароль и обычного пользователя
   - **6. Сводка** — проверить и запустить установку

### Шаг «4. Компоненты установки» — подробно

**Профиль разметки диска:** можно оставить «Использование LVM с EXT4 и отдельным /home» (подходит для большинства случаев, если задание не требует отдельный раздел именно под `/srv`).

**Программное обеспечение — снять галки:**
- ❌ Графический интерфейс Fly *(это и есть GUI — главное, что отключаем)*
...
- ✅ Средства филь­трации сетевых пакетов ufw (опционально, не помешает)

**Добавить (рекомендуется):**
- ✅ Средства удалённого подключения SSH — удобно для администрирования с другого ПК

**Параметры безопасности — рекомендуется СНЯТЬ:**
- ❌ **Мандатный контроль целостности** — без явной необходимости его лучше отключить, иначе при каждом входе будет запрос `Integrity level`, и могут возникать ошибки `Permission denied` при работе с файлами `/etc/`, `/srv/` без явного выставления нужного уровня/метки через `pdpl-*` утилиты. Задание не требует работы с PARSEC — только обычные права POSIX.

Остальные параметры безопасности (Замкнутая программная среда, Запрет консоли и т.п.) — **не включать**, чтобы не усложнять себе работу в терминале.
